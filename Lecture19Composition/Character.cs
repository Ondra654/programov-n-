...
namespace Lecture19Composition
{
	class Character
	{
		...
		private void AdvantageAttack(TextWriter output, int attackRoll, Character enemy, Die die)
        {
			...
			int randdmg = randadvantage.Next(11, 16);
			int advantagedmg = randdmg - defenceRoll;

			if (advantagedmg > 0)
			{
				hp -= advantagedmg;
				Console.WriteLine("{0} takes advatage and attacks {1}:",enemy.Name, name);
				output.WriteLine("{0} takes {1} damage!", name, advantagedmg);
			}
			else
			{
				Console.WriteLine("{0} takes advatage but also did 0 damage!", enemy.Name);
